...
### index.php
> This index.php file acts as the gateway to our API. All requests are first handled by this file. If you start a PHP developemental server without specifying a file, all requests will default to index.php. To make sure that everything goes smoothly however, we recomend to assign index.php as the root file when starting the server (See [Start servers/api]https://github.com/tiffanyeo/PentaCal?tab=readme-ov-file#api).
